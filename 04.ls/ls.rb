...
class LongOption
  def output(files)
    ...
    hard_link_width = @stats.map { |file, stat| stat.nlink.to_s.length }.max
    file_size_width = @stats.map { |file, stat| stat.size.to_s.length }.max
    last_modified_month_width = @stats.map { |file, stat| stat.mtime.strftime('%-m月').length }.max
